...
result build_sam_configuration_command() { //生成唤醒命令
    // 指令字段（包含 TFI）
    uint8_t payload[] = {0xD4,0x14,0x01};
    int payload_len = sizeof(payload);
    result r =  build_template(payload,payload_len);
    static uint8_t final_payload[24]={0};
    for(int i=0;i<2;i++){
        final_payload[i] = 0x55;
    }
    for(int i=2;i<14;i++){
        final_payload[i] = 0x00;
    }
    for(int i=14;i<24;i++){
        final_payload[i] = r.data[i-14];
    }
    result final;
    final.len = 24;
    final.data = final_payload;
    free(r.data);
    return final;
}
...
result build_template(const uint8_t* payload,int payload_len) {
    result r;
